...
(eval-when(:compile-toplevel :load-toplevel :execute)
  (defun bind-of-handler-bind(bind)
    (destructuring-bind(condition-name slots)bind
      (let((arg(gensym"CONDITION")))
	`(,condition-name(LAMBDA(,arg)
			   (WITH-SLOTS,(mapcar #'car slots),arg
			     (SETF,@(loop :for elt :in slots :append elt))))))))
...
(defun enable(char)
  (with-handler-slot-replace((symbol-not-found((api 'enable)
					       (format-control "Function for dispatch macro character ~S is not found."))))
    (let((reader-name(reader-name char))
	 (reader (get-dispatch-macro-character #\# char)))
      (if reader ; somebody already set it, but...
	(if(eq reader-name (millet:function-name (coerce reader 'function))) ; it's me!
	  #0=(set-dispatch-macro-character #\# char reader-name)
	  (when(y-or-n-p"DEV-TOOLS: #~A reader macro already used. : ~S~&Really do you want to replace it?" char reader)
	    #0#))
	#0#)))) ; nobody set it.
