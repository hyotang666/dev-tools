...
(defun step-expand(form &optional rest)
  (multiple-value-bind(form expandedp)(macroexpand-1 form)
    (prog()
      (progn (print form)(force-output))
      :rec
      (when rest
	(format *query-io* "~&Rest : ")
	(write rest :length 2 :lines 1 :level 2 :pretty t :circle t :escape t
	       :stream *query-io*)
	(force-output))
      :invalid
      (case(prompt-for T "~&~:[~;[E]xpand, ~]~:[~;[D]iscard, ~]~:[~;[S]tep, ~][Q]uit,>> "  expandedp rest(consp form):by #'read-char)
	((#\q #\Q)(return form)) ; Quit.
	((#\d #\D) ; Discard current FORM.
	 (if rest
	   (progn (psetf form(print(car rest)*query-io*)
			 rest (cdr rest)
			 expandedp t)
		  (force-output)
		  (go :rec))
