...
(defun generate(name &optional parent)
  ...
  (let((*default-pathname-defaults* (mkdir name (when parent
...
(defun mkdir(name parent)
  (ensure-directories-exist (mkpath name parent)
			    :verbose T))
...
(defun generate-asd (name)
  (with-generate((make-pathname :name name :type "asd"
				:defaults *default-pathname-defaults*))
    (%generate-asd name)))

(defun %generate-asd(name)
  (let((*package*(find-package :asdf)))
    (format t "; vim: ft=lisp et~%~(~S~)~%~(~:S~)"
	    `(in-package :asdf)
	    `(asdf:defsystem ,name :depends-on ("endaira") :components ((:file ,name))))))

(defun generate-lisp(name)
  (with-generate((make-pathname :name name :type "lisp"
				:defaults *default-pathname-defaults*))
    (%generate-lisp name)))
...
(defun generate-readme(name &optional (*default-pathname-defaults* *default-pathname-defaults*))
  (with-generate((make-pathname :name "README" :type "md"
				:defaults *default-pathname-defaults*))
    (%generate-readme name)))
